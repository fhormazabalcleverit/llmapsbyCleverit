...
const Process = () => {
  ...
  return (
    <section className="bg-backblack py-20 relative overflow-hidden">
      {/* Background SVG */}
      <div className="absolute inset-0 bg-cover bg-center bg-no-repeat opacity-30" style={{ backgroundImage: 'url(/mastra.svg)' }}></div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Process Steps */}
        <div ref={cardsRef} className={`flex flex-col lg:flex-row gap-6 items-stretch fade-in-up-delay  ${cardsVisible ? 'animate' : ''}`}>
          {steps.map((step, index) => (
            <div
              key={index}
              className="group relative bg-gray-900/50 backdrop-blur-md border border-gray-700 hover:border-pink-500/50 rounded-2xl p-6 lg:p-6 transition-all duration-500 ease-in-out cursor-pointer flex-1 lg:hover:flex-[3] min-h-[200px] lg:min-h-[200px] flex flex-col hover:bg-gray-800/50 bg-gradient-to-r from-blue-500/10 via-sky-600/3 to-purple-500/10">
              <div className="mb-1">
                 {/*

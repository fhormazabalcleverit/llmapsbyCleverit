        ...
        <div ref={cardsRef} className={`grid grid-cols-1 lg:grid-cols-3 gap-8 fade-in-up-delay ${cardsVisible ? 'animate' : ''}`}>
          {steps.map((step, index) => (
            <div key={index} className="flex flex-col">
              {/* Card */}
              <div className="group relative border border-gray-800 bg-blue hover:border-pink-500/50 rounded-2xl p-0 transition-all duration-500 ease-in-out min-h-[200px] flex items-center justify-center mb-6">
                {index === 1 ? (
